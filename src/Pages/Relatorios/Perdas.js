...
export default function Perdas({cattle,getCattle}){
    useEffect(getCattle,[]);
    const sortedCattle = () => { return cattle.filter((cow) => cow.morreu)
      ...
      <tbody>
        {(sortedCattle()).map((cow) => {
          return (
            <tr key={cow._id}>
              <td>{cow.brinco}</td>
              <td><Link to={`/gado/${cow._id}`}>{cow.nome}</Link></td>
              <td>{cow.sexo}</td>
              <td>{cow.dtMorte ? formatDate(cow.dtMorte):`-`}</td>
              <td>{cow.causaMorte}</td>
              <td>{cow.pesagem.length>0 ? cow.pesagem[cow.pesagem.length - 1].peso :`-`}</td>
              <td>{cow.pesagem.length>0 ? formatDate(cow.pesagem[cow.pesagem.length - 1].dtpesagem) : `-`}</td>
            </tr>
          );
        })}
      </tbody>
